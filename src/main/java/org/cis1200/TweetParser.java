...
public class TweetParser {
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (iterator.hasNext()) {
            String next = extractColumn(iterator.next(), tweetColumn);
            if (next != null) {
                tweets.add(next);
            }
        }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        String temp = removeURLs(tweet);
        List<String> tempList = tweetSplit(temp);
        List<List<String>> sentences = new LinkedList<>();
        for (String var : tempList) {
            List<String> sentence = parseAndCleanSentence(var);
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        for (String var : tweets) {
            List<List<String>> tweet = parseAndCleanTweet(var);
            if (tweet.size() > 0 && tweet != null) {
                trainingData.addAll(tweet);
            }
        }
